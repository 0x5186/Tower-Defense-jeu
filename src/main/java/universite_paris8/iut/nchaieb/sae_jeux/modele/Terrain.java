...
public class Terrain {
    ...
    public void terrainPlainesCode(){
        for(int i=0; i<6; i++){
            for(int j=0; j<48; j++) { codeTuiles[i][j] = 1; }
        }
        for(int j=0; j<48; j++) { codeTuiles[6][j] = 5; }
        for(int i=7; i<25; i++){
            for(int j=0; j<48; j++) { codeTuiles[i][j] = 3; }
        }
        for(int i=0; i<19; i++){
        ...
        for(int i=11; i<13; i++){
            for(int j=0; j<48; j++) { codeTuiles[i][j] = 6; }
        }
        for(int j=0; j<48; j++) { codeTuiles[10][j] = 7; }
        for(int j=0; j<48; j++) { codeTuiles[13][j] = 8; }
    }
    ...
    public void test() {
        for(int i=0; i<25; i++){
            for(int j=0; j<48; j++) { codeTuiles[i][j] = 0; }
        }
    }
    ...
    public boolean estPraticable(int x, int y){
        ...
        return typeTuile == 6 || typeTuile == 7 || typeTuile == 8;
    }
}